...
class EventCell: BaseCell {
    ...
    var event: Event? {
        didSet {
            ...
            if let channelName = event?.channel?.name, let numCompanies = event?.numCompanies {
     
                let subtitleText = "\(String(describing: channelName)) • \(String(describing: numCompanies)) Companies Attending •  2 days ago"
