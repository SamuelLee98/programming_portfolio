...
double** matrix_allocate(int rows, int cols){ 
	...
	return matrix; //Replace this line. This included to make the skeleton compile
}

void matrix_delete(double** M, int rows){
	//delete a 2D array with 'rows' #of rows
	for (int i=0; i<rows; i++) //delete columns?
	{
		delete[] M[i];
	}
}
...
double** matrix_add(double** A, double** B, int rows, int cols){
	...
	double** R = matrix_allocate(rows, cols);
	...
	return R; //Replace this line. This included to make the skeleton compile
}
...
double** matrix_multiply(double** A, int rows_A, int cols_A, double** B, 
    ...
    double** R = matrix_allocate(rows_A, cols_B);
